...
namespace App\Http\Controllers\Album\Admin\Category;
...
class CategoryController extends Controller
{
    ...
    {
        return view('album.admin.category.index');
    }
    ...
    {
        $form = ['action' => route('admin.album.category.store')];
        ...
        return view('album.admin.category.form', compact('form', 'categories'));
    }
    ...
    public function edit($id)
    {
        ...
        $form = [
            'action' => route('admin.album.category.update', $category['id']),
            'method' => 'put'
        ];
        ...
        return view('album.admin.category.form', compact('category', 'form', 'categories'));
    }

    public function update(Request $request, $id)
    {
        $category = Category::where('category_type', Album::class)->findOrFail($id);
    ...
    public function destroy($id)
    {
        ...
        $categories = Category::with('children')->where('category_type', Album::class);
    ...
    {
        $rules = [
            'slug' => 'required|max:70|unique:categories,slug,null,id,category_type,' . Album::class . '|regex:/(^[A-Za-z-_ ]+$)+/',
            'title' => 'required|max:70',
        ];

        if ($request->method() === 'PUT')
            $rules['slug'] = 'required|regex:/(^[A-Za-z-_ ]+$)+/|unique:categories,slug,' . $category->id . ',id,category_type,' . Album::class;
